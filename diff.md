...
# descriptions.go
Change: some old url, now demo
Merge: Keep modern demo value

# mod.go
Add github.com/hyperledger/aries-framework-go dependencies

# some file (irmac.go?)
Remove old servercore dependency

# irmaconfig.go
Change: much old code added (I think)
Merge: discarded change

# legacy.go
Change: Add parseLDContext func
```go
...
# vc.go
Changed NewCredential to ParseCredential
Changed NewPresentation to ParsePresentation

# server/api.go
Change: maybe add privatkey and HavePrivateKeys func?
```go
...
```
Merge: Try without, can always add them later

# irmaserver
deleted main.go because old file but problem, lot of functions added in main.go
```go
